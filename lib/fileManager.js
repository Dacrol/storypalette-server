...
module.exports = function(app, config) {
  ...
  // Returns the ogg version of a filenamt
  // E.g. 'klapp.mp3' -> 'klapp.ogg'
  function isOgg(fileName) {
    return path.extname(fileName).toLowerCase() === '.ogg';
  } 
  ...
  var middleware = {
    ...
    getSoundOld: function(req, res) {
      console.log('get sound', req.params);

      //var params = req.params[0].split('/');
      var path = req.params.id + '.' + req.params.ext;

      //if (!utilExist(params[1])) {
        if (!pipeFile(resourcesDir + '/sound/' + path,res,req)) {
          res.end();
        }
        return;
      //}
