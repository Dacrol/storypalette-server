    ...
    postFile: function(req, res) {
      ...
      if(req.xhr) {
        ...
        async.waterfall([
          ...
          function (callback) {
              try {
                  var ws = fs.createWriteStream(localPathRaw, function (err) {

                      if (err) {
                          console.log("unable to create write stream");
              ...
              {
                  console.log("unable to transfer file to path %s", localPathRaw);
                  console.log(err.message);
                  return;
              }
          },
