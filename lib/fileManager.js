    ...
    postFile: function(req, res) {
      ...
      if(req.xhr) {
        ...
        async.waterfall([
          ...
          function (callback) {
              try {
                  var ws = fs.createWriteStream(localPathRaw, (err) => {
                      if (err) throw err;
                      console.log('created writestream for path %s', localPathRaw);
                  });
              ...
              {
                  console.log("unable to transfer file to web server")
          ...
          function (callback) {
            ...
            if (ext2type[extension] == "sound") {
                ...
                try {
                    ...
                    console.log("unable to convert sound to ogg");
                    console.log(err.message);
                }
            } else {
              callback();
          ...
          // upload to amazon
          function(callback) {
              var body = fs.createReadStream(localPath, (err) => {
                  if (err) throw err;
                  console.log('created readstream for path %s', localPath);
              });
