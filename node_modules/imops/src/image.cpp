...
image::init(Handle<Object> target) {
    ...
    NODE_SET_PROTOTYPE_METHOD(constructor_template, "colormean", colormean);
...
Handle<Value>
image::colormean(const Arguments& args) {
    ...
    for (unsigned int y=0 ; y < height ; y+=stepY)
    {
        ...
        for (unsigned int x=0 ; x < width ; x+=stepX)
        {
            ...
            rm += src_pixels[FI_RGBA_RED];
            bm += src_pixels[FI_RGBA_BLUE];
            gm += src_pixels[FI_RGBA_GREEN];
            
            src_pixels += (4 * stepX);
        }
        
        src_bits += (spitch * stepY);
    }
    ...
    result->Set(Number::New(0), Number::New(rm));
    result->Set(Number::New(1), Number::New(gm));
    result->Set(Number::New(2), Number::New(bm));
